...
\begin{tikzpicture}[
  ...
  block/.style={
    ...
    rounded corners=2pt,
    minimum height=12mm,
    text width=18mm,      % 固定幅で箱の幅を揃える
    node distance=15mm,
    align=center
  },
  lbl/.style={
    midway, above, yshift=2pt,
    fill=white, inner sep=1pt % ラベルの白縁で線を見やすく
  }
]
\node[block] (raspi) {CNCjs\\ on RasPi};
\node[block, right=of raspi] (esp32) {ESP32\\FluidNC};
\node[block, right=of esp32] (driver) {ステッピング\\ドライバ};
\node[block, right=of driver] (motor) {ステッピング\\モータ};

\draw[->, semithick]
  (raspi) -- node[lbl]{USB} (esp32);
\draw[->, semithick]
  (esp32) -- node[lbl]{制御信号} (driver);
\draw[->, semithick]
  (driver) -- node[lbl]{駆動電流} (motor);
\end{tikzpicture}
\caption{本CNCの概念図}
\label{fig:minimal}
\end{figure}
...
\bibliographystyle{unsrt} % 出現順で番号付け
\bibliography{refs}       % refs.bib を使う
